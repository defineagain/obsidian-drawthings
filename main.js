...
var PlotbeatProcessor = class {
  ...
  queue;
  charResolver;
  constructor(app, settings, queue, charResolver) {
    this.app = app;
  ...
  async process(source, el, ctx) {
    ...
    const fileBasename = currentFile instanceof import_obsidian3.TFile ? currentFile.basename : "Scene";
  ...
  renderCard(container, beat, ctx) {
    ...
    btnCopyLink.addEventListener("click", () => {
      ...
      new import_obsidian3.Notice(`Copied: ${embedMarkdown}`);
    });
...
var SceneScriptProcessor = class {
  ...
  renderSceneDeck(container, sceneData, ctx) {
    ...
    topBarRight.createSpan({ cls: "drawthings-badge model-badge", text: `\u{1F4E6} ${sceneData.model || this.settings.defaultModel}` });
    ...
    btnGenerateAll.addEventListener("click", async () => {
      new import_obsidian4.Notice(`Enqueuing ${sceneData.beats.length} beats for "${sceneData.scene}"...`);
      for (const beat of sceneData.beats) {
        const job = await this.buildJobForBeat(beat, ctx.sourcePath);
        this.queue.enqueue(job);
      }
...
var BeatReviewModal = class extends import_obsidian5.Modal {
  sceneName;
  preset;
  beats;
  targetFile;
  ...
  async insertSceneScript(atTop) {
    ...
    if (activeBeats.length === 0) {
      new import_obsidian5.Notice("No beats selected.");
      return;
    }
    const scriptObj = {
...
`;
    ...
    new import_obsidian5.Notice(`Inserted scene script block into ${this.targetFile.basename}!`);
    this.close();
  }
...
`;
    ...
    new import_obsidian5.Notice(`Inserted ${activeBeats.length} inline plotbeats into ${this.targetFile.basename}!`);
    this.close();
  }
  async queueSelectedBeats() {
    ...
    new import_obsidian5.Notice(`Queued ${activeBeats.length} plates for generation!`);
  }
};

// src/storyboardView.ts
var STORYBOARD_VIEW_TYPE = "drawthings-storyboard-view";
var StoryboardView = class extends import_obsidian6.ItemView {
  queue;
  settings;
  charResolver;
  llmClient;
  ...
  constructor(leaf, queue, settings, charResolver, llmClient) {
    super(leaf);
    this.queue = queue;
    this.settings = settings;
    this.charResolver = charResolver;
  ...
  async refresh() {
    ...
    btnGenAll.addEventListener("click", async () => {
      new import_obsidian6.Notice(`Queueing ${beats.length} beats from ${file.basename}...`);
  ...
  async triggerAiScripting(file, content, buttonEl) {
    ...
    if (!file) {
      new import_obsidian6.Notice("Please select a scene note first.");
      return;
    }
    let origText = "";
    if (buttonEl) {
    ...
    new import_obsidian6.Notice("\u{1F916} Analyzing scene text & extracting visual beats with AI...");
    try {
      ...
      if (parsed.beats.length === 0) {
        new import_obsidian6.Notice("No beats were returned by AI.");
        return;
      }
      new BeatReviewModal(
        this.app,
      ...
      new import_obsidian6.Notice(`\u274C AI Beat Extraction failed: ${err.message}`, 8e3);
  ...
  async insertTemplate(file) {
    ...
    if (!file) {
      new import_obsidian6.Notice("Please select an active note first.");
      return;
    }
    const template = `
...
`;
    try {
      ...
      new import_obsidian6.Notice(`Inserted plotbeat template into "${file.basename}"!`);
      await this.refresh();
    } catch (e) {
      new import_obsidian6.Notice(`Failed to insert template: ${e.message}`);
    }
  }
...
var LLMClient = class {
  ...
  async callOllamaWithEndpoint(endpoint, model, systemPrompt, userPrompt) {
    ...
    try {
      const res = await (0, import_obsidian7.requestUrl)(req);
      const data = res.json;
...
// src/main.ts
var DrawThingsPlugin = class extends import_obsidian8.Plugin {
  ...
  async onload() {
    ...
    this.addCommand({
      ...
      callback: async () => {
        ...
        if (!view || !view.file) {
          new import_obsidian8.Notice("Please open a scene note first.");
          return;
        }
        ...
        try {
          ...
          const parsed = parseBeatsResponse(response, file.basename);
          if (parsed.beats.length === 0) {
            new import_obsidian8.Notice("No beats returned by AI.");
            return;
          }
          new BeatReviewModal(
            this.app,
          ...
          new import_obsidian8.Notice(`\u274C AI Beat Extraction failed: ${err.message}`);
          console.error(err);
        }
      }
    });
    this.addCommand({
      ...
      editorCallback: async (editor, view) => {
        ...
        if (!selected || selected.trim().length === 0) {
          new import_obsidian8.Notice("Please select scene prose in the editor first.");
          return;
        }
        new import_obsidian8.Notice("\u{1F916} Crafting visual prompt from selected text...");
        try {
          const { systemPrompt, userPrompt } = buildBeatExtractionPrompt(
...
`;
            ...
            new import_obsidian8.Notice("Inserted plotbeat block!");
          }
        } catch (err) {
          new import_obsidian8.Notice(`\u274C AI Beat generation failed: ${err.message}`);
        }
      }
    });
    this.addCommand({
